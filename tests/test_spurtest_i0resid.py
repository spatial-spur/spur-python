...
def run_stata_spurtest_i0resid(
    tmp_path: Path,
    df: pd.DataFrame,
) -> tuple[float, float]:
    ...
    input_csv = tmp_path / "spurtest_i0resid_input.csv"
    output_csv = tmp_path / "spurtest_i0resid_output.csv"
    ...
    script = textwrap.dedent(
        ...
        _spurtest_i0resid am fracblack, q(10) nrep({NREP}) latlong
...
def test_spurtest_i0resid_returns_valid_result() -> None:
...
def test_spurtest_i0resid_matches_stata(
    tmp_path: Path,
    chetty_df: pd.DataFrame,
) -> None:
    py_value = spurtest_i0resid(
        "am ~ fracblack",
        chetty_df,
        lon="lon",
        lat="lat",
    ...
    st_LR, st_ha = run_stata_spurtest_i0resid(tmp_path, chetty_df)
