...
from spur import load_chetty_data, spurtest_i1resid, standardize
...
def run_stata_spurtest_i1resid(
    tmp_path: Path,
    df: pd.DataFrame,
) -> tuple[float, float]:
    ...
    input_csv = tmp_path / "spurtest_i1resid_input.csv"
    output_csv = tmp_path / "spurtest_i1resid_output.csv"
...
def test_spurtest_i1resid_returns_valid_result() -> None:
...
def test_spurtest_i1resid_matches_stata(
    tmp_path: Path,
    chetty_df: pd.DataFrame,
) -> None:
    py_value = spurtest_i1resid(
        "am ~ fracblack",
        chetty_df,
        lon="lon",
        lat="lat",
    ...
    st_LR, st_ha = run_stata_spurtest_i1resid(tmp_path, chetty_df)
