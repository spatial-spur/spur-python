...
from spur.utils.matrix import lbmgls_matrix
...
def run_stata_lbmgls_matrix(tmp_path: Path, coords: np.ndarray) -> np.ndarray:
    stata_root = ensure_spur_stata_installed()
    plus = stata_root / "plus"
    personal = stata_root / "personal"
    input_csv = tmp_path / "lbmgls_input.csv"
    output_csv = tmp_path / "lbmgls_output.csv"
    ...
    script = textwrap.dedent(
        ...
        spurtransform {varlist}, prefix(m_) transformation(lbmgls) latlong
...
def test_lbmgls_matrix_is_symmetric_and_rejects_identical_coords(
    grid_coords: np.ndarray,
) -> None:
    matrix = lbmgls_matrix(grid_coords, latlon=True)

    npt.assert_allclose(matrix, matrix.T, atol=ATOL, rtol=RTOL)

    with pytest.raises(ValueError, match="identical"):
        lbmgls_matrix(np.zeros((5, 2)), latlon=False)


@pytest.mark.skipif(STATA is None, reason="stata-mp not installed")
def test_lbmgls_matrix_matches_stata(
    tmp_path: Path,
    grid_coords: np.ndarray,
) -> None:
    py_value = lbmgls_matrix(grid_coords, latlon=True)
    st_value = run_stata_lbmgls_matrix(tmp_path, grid_coords)
